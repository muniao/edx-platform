...
class LogoutTests(TestCase):
    ...
    def test_learner_portal_logout_having_idp_logout_url(self):
        ...
        mock_idp_logout_url = 'http://mock-idp.com/logout'
        client = self._create_oauth_client()

        with mock.patch(
            'openedx.core.djangoapps.user_authn.views.logout.tpa_pipeline.get_idp_config_from_running_pipeline'
        ) as mock_idp_config:
            mock_idp_config.return_value.conf = {
                'logout_url': mock_idp_logout_url
            }
            response = self._assert_session_logged_out(client, HTTP_REFERER=learner_portal_logout_url)
            expected = {
                'tpa_logout_url': mock_idp_logout_url,
